...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def draw_bboxes_3d(self,
                       ...
                       bboxes_3d_2: BaseInstance3DBoxes,
                       gt_bboxes_3d: BaseInstance3DBoxes,
                       p_bbox_color: Tuple[float] = (0, 0, 1.0),  # pre_box  blue
                       p_bbox_color_2: Tuple[float] = (0, 0.5, 0.5),  # green
                       bbox_color: Tuple[float] = (1.0, 0, 0),  # red
                       points_in_box_color: Tuple[float] = (0, 1.0, 0),  #green
                       rot_axis: int = 2,
        ...
        check_type('bboxes', gt_bboxes_3d, BaseInstance3DBoxes)
        if not isinstance(gt_bboxes_3d, DepthInstance3DBoxes):
            gt_bboxes_3d = gt_bboxes_3d.convert_to(Box3DMode.DEPTH)
        # convert bboxes to numpy dtype
        check_type('bboxes', bboxes_3d_2, BaseInstance3DBoxes)
        if not isinstance(bboxes_3d_2, DepthInstance3DBoxes):
            bboxes_3d_2 = bboxes_3d_2.convert_to(Box3DMode.DEPTH)

        bboxes_3d = tensor2ndarray(bboxes_3d.tensor)
        gt_bboxes_3d = tensor2ndarray(gt_bboxes_3d.tensor)
        bboxes_3d_2 = tensor2ndarray(bboxes_3d_2.tensor)

        in_box_color = np.array(points_in_box_color)

        for i in range(len(gt_bboxes_3d)):
            center = gt_bboxes_3d[i, 0:3]
            dim = gt_bboxes_3d[i, 3:6]
            yaw = np.zeros(3)
            yaw[rot_axis] = gt_bboxes_3d[i, 6]
            ...
            if self.pcd is not None and mode == 'xyz':
                ...
                self.points_colors[indices] = np.array(in_box_color)
            # if num_points >5:
            #     bbox_color = bbox_color_c
            # print(num_points)
        ...
        for i in range(len(bboxes_3d)):
            ...
            line_set.paint_uniform_color(np.array(p_bbox_color))
            # draw bboxes on visualizer
            self.o3d_vis.add_geometry(line_set)

            #change the color of points which are in box
        for i in range(len(bboxes_3d_2)):
            center = bboxes_3d_2[i, 0:3]
            dim = bboxes_3d_2[i, 3:6]
            yaw = np.zeros(3)
            yaw[rot_axis] = bboxes_3d_2[i, 6]
            rot_mat = geometry.get_rotation_matrix_from_xyz(yaw)
            ...
            line_set.paint_uniform_color(np.array(p_bbox_color_2))
            # draw bboxes on visualizer
            self.o3d_vis.add_geometry(line_set)
        ...
        x3 = radius3 * np.cos(theta)
        y3 = radius3 * np.sin(theta)
        z1 = np.zeros_like(x1)  # 圆圈位于z=0的平面上
        z2 = np.zeros_like(x2)
        z3 = np.zeros_like(x3)
        # 创建点云对象
        points1 = np.vstack((x1, y1, z1)).T
        points2 = np.vstack((x2, y2, z2)).T
        points3 = np.vstack((x3, y3, z3)).T
        ...
        line_set4 = o3d.geometry.LineSet(
            points=o3d.utility.Vector3dVector(points3),
            lines=o3d.utility.Vector2iVector(lines)
        )
        # 设置圆圈的颜色
        line_set2.paint_uniform_color([0, 1, 0])  # 设置为绿色
        line_set3.paint_uniform_color([0, 1, 0])
        line_set4.paint_uniform_color([0, 1, 0])
        # 将圆圈添加到三维场景中
        self.o3d_vis.add_geometry(line_set2)
        self.o3d_vis.add_geometry(line_set3)
        self.o3d_vis.add_geometry(line_set4)
