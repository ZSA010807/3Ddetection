...
class Visualizer2D:
    ...
    def handler_pc(self, pc, color='gray', s=0.25):
        vis_pc = np.asarray(pc)
        plt.scatter(vis_pc[:, 0], vis_pc[:, 1], s=s, marker='o', color=self.COLOR_MAP[color])
    ...
    def handler_box(self, box: BBox, message: str='', color='red', linestyle='solid', text_color=None, fontsize='xx-small', center_message=False):
        ...
        if text_color is None:
            text_color = color 
        if center_message:
            plt.text(box.x - 1, box.y, message, color=self.COLOR_MAP[text_color], fontsize=fontsize)
        else:
...
class Visualizer3D:
    ...
    def handler_pc_3d(self, pc, color='gray', s=0.25):
        # 将点云数据转换为NumPy数组
        vis_pc = np.asarray(pc)
        ...
        # s: 点的大小
        # color: 点的颜色
        ax.scatter3D(vis_pc[:, 0], vis_pc[:, 1], vis_pc[:, 2], s=s, marker='o', color=self.COLOR_MAP[color])
    ...
    def handler_box_3d(self, box, message='', color='red', linestyle='solid', text_color=None, fontsize='xx-small', center_message=False):
        corners = np.array(BBox.box2corners3d(box))  # 假设box2corners3d返回包含3D坐标的数组
        corners = np.concatenate([corners, corners[0:1]])

        # 使用plot绘制3D边界框的线
        self.ax.plot(corners[:, 0], corners[:, 1], corners[:, 2], color=color, linestyle=linestyle)
        corner_index = np.random.randint(0, 4, 1)
        if text_color is None:
            text_color = color

        if center_message:
            self.ax.text(box.x - 1, box.y, box.z, message, color=text_color, fontsize=fontsize)
        else:
            self.ax.text(corners[corner_index, 0] - 1, corners[corner_index, 1] - 1, corners[corner_index, 2] - 1, message, color=text_color, fontsize=fontsize)