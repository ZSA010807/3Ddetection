...
def frame_visualization_3d(pc, dets, gts, name='', save_path='./exp.png', figsize=(40, 40)):
    visualizer = Visualizer3D(name=name, figsize=figsize)
    visualizer.handler_pc_3d(pc, s=0.1)
    if gts is not None:
        for _, bbox in enumerate(gts):
            visualizer.handler_box_3d(bbox, message='', color='black')
    for det in dets:
        # visualizer.handler_box(det, message='%.2f-' % det.s + str(int(det.type)), color='red', linestyle='dashed', text_color='green', fontsize='small')
        visualizer.handler_box_3d(det, message='%.2f' % det.s, color='red', linestyle='dashed', text_color='blue', fontsize='small', center_message=True)
    visualizer.save(save_path)
    visualizer.close()
...
if __name__ == '__main__':
    ...
    if args.save_folder == '':
        save_folder = osp.join(osp.dirname(bin_path), 'vis_folder_3d')
    ...
    for i, ts in tqdm(enumerate(ts_list)):
        ...
        if gt_dict is None:
            ...
            gts = gt_dict[ts]
        # set_trace()
        pc = get_pc_from_time_stamp(ts, '/data/zhaoshenao/code/SST/data/waymo/kitti_format/idx2timestamp.pkl', split=args.split)
        # set_trace()
